...
jQuery(document).ready(function($) {
 ...
 siteScroll();
   ...
   function toggleComments() {
	   ...
	   commentsContainer.style.display = commentsVisible ? "block" : "none"; // Show or hide the comments container based on the visibility state
   }
   
   // Prevent toggling when clicking on the input or button inside the comments container
   document.getElementById("commentsContainer").addEventListener("click", function(event) {
	   event.stopPropagation();
   });
   
   // Add event listener to toggle comments when clicking on the toggle-comments element
   document.querySelector(".toggle-comments").addEventListener("click", function(event) {
	   toggleComments();
...
if (urlParams.get('showQuestion') === 'true') {
   // Call the showQuestionForm() function
   showQuestionForm();
}
...
function validerFormulaire(event) {
   ...
   var id_userInput = document.getElementById("id_user");
   var id_user = id_userInput.value;
   var id_userError = document.getElementById("user_idError");
   ...
   // Expression régulière pour vérifier si l'ID utilisateur ne contient que des chiffres et a une longueur maximale de 8 caractères
   var regex = /^\d{1,8}$/;
   
   var id_userValid = regex.test(id_user);
   var titreValid = titre.trim() !== "";
   var contenuValid = contenu.trim() !== "";
   ...
   if (!id_userValid) {
	   if (id_user.length === 0) {
		   ...
		   id_userError.innerText = "L'ID utilisateur ne doit pas dépasser 8 chiffres.";
	   } else {
		   id_userError.innerText = "L'ID utilisateur ne doit contenir que des chiffres.";
	   }
   ...
   // Si tous les champs sont valides, soumettre le formulaire et cacher le formulaire
   if (id_userValid && titreValid && contenuValid) {
